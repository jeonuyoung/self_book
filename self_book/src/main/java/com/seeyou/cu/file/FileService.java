...
public class FileService {
	...
	public static String saveFile(MultipartFile mfile, String uploadPath,String id) {
		...
		uploadPath = "C://Users/cool/git/self_book/self_book/src/main/webapp/resources/demo/"+id;
		//저장 폴더가 없으면 생성
		File path = new File(uploadPath);
		if (!path.isDirectory()) {		//저장 폴더가 없으면
			path.mkdirs();				//생성
		}
		
		//원본 파일명
		String originalFilename = mfile.getOriginalFilename();		//getOriginalFilename()는 원래 존재하는 메소드
		
		//저장할 파일명을 오늘 날짜의 년월일로 생성 (파일명이 겹치는 것을 막기 위해 날짜 사용)
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		String savedFilename = sdf.format(new Date());
		
		//원본 파일의 확장자
		String ext;
		int lastIndex = originalFilename.lastIndexOf('.');			//.의 위치를 찾음
		//확장자가 없는 경우
		if (lastIndex == -1) {
			ext = "";
		}
		//확장자가 있는 경우
			...
			ext = "." + originalFilename.substring(lastIndex + 1);	//확장자를 꺼냄
		}
		...
		//★ 파일 저장
		try {
			mfile.transferTo(serverFile);		//스프링에서 사용 가능한 함수
		} catch (Exception e) {
			savedFilename = null;
			e.printStackTrace();
		}
